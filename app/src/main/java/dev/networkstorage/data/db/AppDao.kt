...
interface AppDao {
    ...
    @Query("SELECT * FROM connections WHERE rootMode='MIRROR' ORDER BY name") suspend fun mirrorConnections(): List<ConnectionEntity>
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun upsertEntry(value: IndexedEntryEntity)
    @Query("SELECT indexed_entries.*, cache_entries.localDocumentUri AS cacheDocumentUri, cache_entries.size AS cacheSize, CASE WHEN indexed_entries.isDirectory=1 AND EXISTS(SELECT 1 FROM cache_entries AS descendant_cache WHERE descendant_cache.connectionId=indexed_entries.connectionId AND descendant_cache.state='CACHED' AND descendant_cache.relativePath LIKE CASE WHEN indexed_entries.relativePath='' THEN '%' ELSE indexed_entries.relativePath || '/%' END) THEN indexed_entries.size ELSE cache_entries.remoteSize END AS cacheRemoteSize, CASE WHEN indexed_entries.isDirectory=1 AND EXISTS(SELECT 1 FROM cache_entries AS descendant_cache WHERE descendant_cache.connectionId=indexed_entries.connectionId AND descendant_cache.state='CACHED' AND descendant_cache.relativePath LIKE CASE WHEN indexed_entries.relativePath='' THEN '%' ELSE indexed_entries.relativePath || '/%' END) THEN indexed_entries.lastModified ELSE cache_entries.remoteLastModified END AS cacheRemoteLastModified, CASE WHEN indexed_entries.isDirectory=1 AND EXISTS(SELECT 1 FROM cache_entries AS descendant_cache WHERE descendant_cache.connectionId=indexed_entries.connectionId AND descendant_cache.state='CACHED' AND descendant_cache.relativePath LIKE CASE WHEN indexed_entries.relativePath='' THEN '%' ELSE indexed_entries.relativePath || '/%' END) THEN 'CACHED' ELSE cache_entries.state END AS cacheState FROM indexed_entries LEFT JOIN cache_entries ON cache_entries.connectionId=indexed_entries.connectionId AND cache_entries.relativePath=indexed_entries.relativePath WHERE indexed_entries.connectionId=:connectionId AND indexed_entries.parentPath=:parentPath ORDER BY indexed_entries.isDirectory DESC, indexed_entries.name COLLATE NOCASE ASC") fun children(connectionId: String, parentPath: String): PagingSource<Int, BrowserEntryRow>
