...
private fun displayMode(mode: FolderMode): String = if (mode == FolderMode.MIRROR) "MIRROR" else "ON-DEMAND"
private fun isMirror(mode: FolderMode): Boolean = mode == FolderMode.MIRROR
                ...
                ElevatedCard(Modifier.fillMaxWidth(), shape = SectionShape, colors = CardDefaults.elevatedCardColors(containerColor = MaterialTheme.colorScheme.surfaceContainerLow)) {
                    Column(Modifier.padding(horizontal = 14.dp, vertical = 10.dp), verticalArrangement = Arrangement.spacedBy(CompactGap)) {
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            ...
                            Surface(shape = RoundedCornerShape(999.dp), color = MaterialTheme.colorScheme.secondaryContainer) { Text(displayMode(summary.connection.mode), Modifier.padding(horizontal = 8.dp, vertical = 3.dp), style = MaterialTheme.typography.labelSmall) }
                        }
                        HorizontalDivider()
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                            ...
                            if (isMirror(summary.connection.mode)) OutlinedButton(onClick = { viewModel.openMirror(summary) }, enabled = summary.hasRootRule) { Text("⇄ Sync") }
                            ...
                            DropdownMenu(expanded = menu, onDismissRequest = { menu = false }) {
                                ...
                                if (isMirror(summary.connection.mode)) DropdownMenuItem(text = { Text("Sync / compare") }, onClick = { menu = false; viewModel.openMirror(summary) }, enabled = summary.hasRootRule)
...
private fun SyncScreen(viewModel: MainViewModel) {
    ...
    Column(Modifier.fillMaxSize().padding(horizontal = ScreenPadding)) {
        ...
        if (mirror.loading) { Spacer(Modifier.height(6.dp)); LinearProgressIndicator(Modifier.fillMaxWidth()); Text("Comparing NAS and local mirror…", style = MaterialTheme.typography.bodySmall) }
        mirror.state?.takeIf { !it.isFinished }?.let { Spacer(Modifier.height(6.dp)); Surface(shape = RowShape, color = MaterialTheme.colorScheme.secondaryContainer.copy(alpha = .45f)) { Column(Modifier.fillMaxWidth().padding(10.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) { Text("Syncing ${mirror.currentPath.substringAfterLast('/')}", style = MaterialTheme.typography.bodyMedium); LinearProgressIndicator(progress = { usageRatio(mirror.copied, mirror.currentTotal) }, modifier = Modifier.fillMaxWidth()); Text("${mirror.completedFiles}/${mirror.totalFiles} files  ${formatBytes(mirror.copied)}/${formatBytes(mirror.currentTotal)}", style = MaterialTheme.typography.bodySmall); TextButton(onClick = viewModel::cancelMirrorSync) { Text("Cancel") } } } }
        Spacer(Modifier.height(6.dp)); if (!mirror.loading && mirror.items.isEmpty()) Text("No differences found. Tap Compare after scanning the NAS.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
...
private fun MirrorRow(item: MirrorDiffItem, idle: Boolean, onSync: () -> Unit) {
    ...
    Surface(shape = RowShape, color = MaterialTheme.colorScheme.surfaceContainerLow) { Row(Modifier.fillMaxWidth().padding(horizontal = 10.dp, vertical = 7.dp)) { Text(icon, style = MaterialTheme.typography.titleMedium); Spacer(Modifier.width(8.dp)); Column(Modifier.weight(1f)) { Text(item.name, style = MaterialTheme.typography.bodyMedium, fontWeight = FontWeight.Medium, maxLines = 1, overflow = TextOverflow.Ellipsis); Text("$label • NAS ${item.remoteSize?.let(::formatBytes) ?: "—"} • Mirror ${item.localSize?.let(::formatBytes) ?: "—"}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant, maxLines = 1) }; if (MirrorSyncPolicy.canCopyRemoteToLocal(item.state)) TextButton(onClick = onSync, enabled = idle) { Text("Sync") } } }
}
...
private fun BrowserScreen(viewModel: MainViewModel) {
    ...
    Column(Modifier.fillMaxSize().padding(horizontal = ScreenPadding)) {
        Spacer(Modifier.height(8.dp)); val root = connection?.connection?.let { listOf(it.share, it.basePath).filter(String::isNotBlank).joinToString(" / ") }.orEmpty()
        Text(path.substringAfterLast('/', root.substringAfterLast('/', connection?.connection?.name ?: "Select a connection")), style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold)
        Text(listOfNotNull(connection?.connection?.name, root.takeIf(String::isNotEmpty), path.takeIf(String::isNotEmpty)).joinToString(" / "), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant, maxLines = 1, overflow = TextOverflow.Ellipsis)
        Spacer(Modifier.height(6.dp)); if (path.isNotEmpty()) Surface(shape = RowShape, color = MaterialTheme.colorScheme.surfaceContainer) { Text("← One level up", Modifier.fillMaxWidth().clickable { viewModel.browserBack() }.padding(horizontal = 10.dp, vertical = 8.dp), style = MaterialTheme.typography.bodyMedium, fontWeight = FontWeight.Medium) }
        download.state?.takeIf { !it.isFinished }?.let { Spacer(Modifier.height(6.dp)); Surface(shape = RowShape, color = MaterialTheme.colorScheme.secondaryContainer.copy(alpha = .45f)) { Column(Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) { Text("Downloading ${download.path}", style = MaterialTheme.typography.bodyMedium); LinearProgressIndicator(progress = { usageRatio(download.count, download.total) }, modifier = Modifier.fillMaxWidth()); Text("${formatBytes(download.count)} / ${formatBytes(download.total)} (${downloadPercent(download.count, download.total)}%)", style = MaterialTheme.typography.bodySmall); TextButton(onClick = viewModel::cancelDownload) { Text("Cancel") } } } }
        Spacer(Modifier.height(6.dp)); LazyColumn(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(3.dp)) { items(items.itemCount) { index -> items[index]?.let { item -> BrowserRow(item, { viewModel.openFolder(item) }, { viewModel.removeCache(item) }) } } }
    }
}
...
private fun SettingsScreen(viewModel: MainViewModel) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(horizontal = ScreenPadding), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        item { Spacer(Modifier.height(10.dp)); Text("Settings", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.SemiBold); Text("Cache and mirror storage", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
        item { SettingsSectionCard("☁ On-demand cache") { Text("Storage folder", style = MaterialTheme.typography.labelMedium); StoragePathBox(cacheRoot); OutlinedButton(onClick = { cachePicker.launch(null) }, Modifier.fillMaxWidth()) { Text("▱ Choose folder") }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text("Cache limit", fontWeight = FontWeight.Medium); Text("${bytes / SettingsRepository.BYTES_PER_GIB} GB", color = MaterialTheme.colorScheme.primary) }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(4.dp)) { SettingsRepository.PRESET_GIB.forEach { gib -> if (bytes == gib * SettingsRepository.BYTES_PER_GIB) FilledTonalButton(onClick = { viewModel.setCacheLimitGib(gib.toString()) }, modifier = Modifier.weight(1f)) { Text("$gib GB", maxLines = 1) } else OutlinedButton(onClick = { viewModel.setCacheLimitGib(gib.toString()) }, modifier = Modifier.weight(1f)) { Text("$gib GB", maxLines = 1) } } }; OutlinedTextField(custom, { custom = it }, label = { Text("Custom limit") }, suffix = { Text("GB") }, modifier = Modifier.fillMaxWidth(), singleLine = true); Button(onClick = { viewModel.setCacheLimitGib(custom) }, Modifier.fillMaxWidth()) { Text("Save custom limit") }; Text("Usage ${formatBytes(usage)} / ${formatBytes(bytes)}", style = MaterialTheme.typography.bodySmall); LinearProgressIndicator(progress = { usageRatio(usage, bytes) }, modifier = Modifier.fillMaxWidth()); OutlinedButton(onClick = { confirmClearCache = true }, enabled = usage > 0L, modifier = Modifier.fillMaxWidth()) { Text("Clear On-demand cache") } } }
        item { SettingsSectionCard("▣ Mirror storage") { StoragePathBox(mirrorRoot); OutlinedButton(onClick = { mirrorPicker.launch(null) }, Modifier.fillMaxWidth()) { Text("▱ Choose folder") }; Text("Mirror files are persistent and are never removed by cache cleanup.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) } }
...
@Composable private fun StoragePathBox(uri: String?) { Surface(shape = RoundedCornerShape(10.dp), color = MaterialTheme.colorScheme.surfaceContainer) { Text(friendlyStorageRoot(uri), Modifier.fillMaxWidth().padding(horizontal = 10.dp, vertical = 7.dp), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant, maxLines = 1, overflow = TextOverflow.Ellipsis) } }
...
private fun ConnectionEditorScreen(viewModel: MainViewModel) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(horizontal = ScreenPadding), verticalArrangement = Arrangement.spacedBy(7.dp)) {
        ...
        item { Text("Mode", style = MaterialTheme.typography.labelMedium); Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) { listOf(FolderMode.ON_DEMAND, FolderMode.MIRROR).forEach { candidate -> if ((editor.mode == candidate) || (editor.mode == FolderMode.INDEX_ONLY && candidate == FolderMode.ON_DEMAND)) FilledTonalButton(onClick = { if (editor.mode != candidate) viewModel.updateEditor(editor.copy(mode = candidate)) }) { Text(if (candidate == FolderMode.MIRROR) "Mirror" else "On-demand") } else OutlinedButton(onClick = { viewModel.updateEditor(editor.copy(mode = candidate)) }) { Text(if (candidate == FolderMode.MIRROR) "Mirror" else "On-demand") } } } }
        item { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) { TextButton(onClick = viewModel::showConnections) { Text("Cancel") }; Button(onClick = { viewModel.saveEditor(password); password = "" }, enabled = editor.share.isNotBlank()) { Text("Save") } }; Spacer(Modifier.height(8.dp)) }
    }
