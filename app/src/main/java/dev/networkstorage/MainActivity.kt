                    ...
                    Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            Column {
                                ...
                                Text("${summary.connection.share}${summary.connection.basePath.takeIf(String::isNotBlank)?.let { " / $it" }.orEmpty()}", color = MaterialTheme.colorScheme.onSurfaceVariant)
                            }
    ...
    Column(Modifier.fillMaxSize().padding(horizontal = ScreenPadding)) {
        ...
        LazyColumn(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            items(items.itemCount) { index ->
                items[index]?.let { item -> BrowserRow(item, onClick = { viewModel.openFolder(item) }, onRemoveCache = { viewModel.removeCache(item) }) }
            }
        }
    }
}
...
private fun BrowserRow(item: BrowserItem, onClick: () -> Unit, onRemoveCache: () -> Unit) {
    ...
    var menu by remember(item.relativePath) { mutableStateOf(false) }
    val hasCache = !item.isDirectory && (item.localState == LocalFileState.CACHED || item.localState == LocalFileState.REMOTE_UPDATED)
